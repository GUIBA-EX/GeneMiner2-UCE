...
The default build includes the Rust MainFilter, Refilter, Assembler, Population workflow, and Rust helper tools. `--assembler-implementation auto` tries Rust first and falls back to the unmodified original Python assembler, except in ITS2 mode. The main CLI orchestrator and consensus program remain in Python.
