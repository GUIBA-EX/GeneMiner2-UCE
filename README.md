...
除非文件另有说明，本仓库当前及后续版本的原创 Rust 实现、文档、测试与工作流扩展版权归夏非（Fei Xia）所有，并以 [GPL-3.0-or-later](LICENSE) 发布。项目保留第三方或直接移植代码的原始版权与许可声明；来源边界见 [NOTICE](NOTICE)。使用本软件及其结果发表研究时，请同时引用本项目及相关方法和软件的原始论文。
